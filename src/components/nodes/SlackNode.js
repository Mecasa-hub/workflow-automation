import React from 'react';
import { Handle, useReactFlow } from 'reactflow';
import { Box, TextField, Select, MenuItem, FormControl, InputLabel, Typography, Stack, IconButton } from '@mui/material';
import { Message as SlackIcon } from '@mui/icons-material';
import DeleteIcon from '@mui/icons-material/Delete';

const SlackNode = ({ id, data, isConnectable, updateNodeData }) => {
  const { setNodes, setEdges } = useReactFlow();

  const onDelete = () => {
    setNodes((nodes) => nodes.filter(node => node.id !== id));
    setEdges((edges) => edges.filter(edge => edge.source !== id && edge.target !== id));
  };

  const handleChange = (field) => (event) => {
    updateNodeData({ ...data, [field]: event.target.value });
  };

  return (
    <Box
      sx={{
        background: '#fff',
        padding: '15px',
        borderRadius: '8px',
        border: '1px solid #ddd',
        minWidth: '250px',
      }}
    >
      <Handle type="target" position="top" isConnectable={isConnectable} />
      
      <Stack 
        direction="row" 
        spacing={1} 
        alignItems="center" 
        sx={{ 
          mb: 2,
          pb: 1,
          borderBottom: '1px solid #e0e0e0',
          pr: 4
        }}
      >
        <SlackIcon sx={{ color: '#E01E5A' }} />
        <Typography variant="subtitle2">Slack</Typography>
      </Stack>

      <IconButton 
        size="small" 
        onClick={onDelete}
        sx={{ 
          position: 'absolute',
          top: 12,
          right: 12,
          bgcolor: 'rgba(0,0,0,0.05)',
          '&:hover': { bgcolor: 'rgba(0,0,0,0.1)' }
        }}
      >
        <DeleteIcon fontSize="small" />
      </IconButton>

      <TextField
        fullWidth
        label="Bot Token"
        value={data.botToken || ''}
        onChange={handleChange('botToken')}
        type="password"
        sx={{ mb: 2 }}
      />

      <FormControl fullWidth sx={{ mb: 2 }}>
        <InputLabel>Operation</InputLabel>
        <Select
          value={data.operation || 'sendMessage'}
          onChange={handleChange('operation')}
          label="Operation"
        >
          <MenuItem value="sendMessage">Send Message</MenuItem>
          <MenuItem value="sendFile">Send File</MenuItem>
          <MenuItem value="updateMessage">Update Message</MenuItem>
          <MenuItem value="deleteMessage">Delete Message</MenuItem>
          <MenuItem value="addReaction">Add Reaction</MenuItem>
          <MenuItem value="createChannel">Create Channel</MenuItem>
          <MenuItem value="inviteToChannel">Invite to Channel</MenuItem>
        </Select>
      </FormControl>

      <TextField
        fullWidth
        label="Channel"
        value={data.channel || ''}
        onChange={handleChange('channel')}
        placeholder="Channel name or ID"
        sx={{ mb: 2 }}
      />

      <TextField
        fullWidth
        label="Message Text"
        multiline
        rows={3}
        value={data.messageText || ''}
        onChange={handleChange('messageText')}
        placeholder="Enter message text"
        sx={{ mb: 2 }}
      />

      <TextField
        fullWidth
        label="Thread TS"
        value={data.threadTs || ''}
        onChange={handleChange('threadTs')}
        placeholder="Thread timestamp (optional)"
        sx={{ mb: 2 }}
      />

      <TextField
        fullWidth
        label="File URL"
        value={data.fileUrl || ''}
        onChange={handleChange('fileUrl')}
        placeholder="URL of file to share"
        sx={{ mb: 2 }}
      />

      <TextField
        fullWidth
        label="File Title"
        value={data.fileTitle || ''}
        onChange={handleChange('fileTitle')}
        placeholder="Title for the file"
        sx={{ mb: 2 }}
      />

      <FormControl fullWidth sx={{ mb: 2 }}>
        <InputLabel>Message Format</InputLabel>
        <Select
          value={data.messageFormat || 'plain'}
          onChange={handleChange('messageFormat')}
          label="Message Format"
        >
          <MenuItem value="plain">Plain Text</MenuItem>
          <MenuItem value="mrkdwn">Markdown</MenuItem>
          <MenuItem value="blocks">Block Kit</MenuItem>
        </Select>
      </FormControl>

      <TextField
        fullWidth
        label="Blocks JSON"
        multiline
        rows={4}
        value={data.blocks || ''}
        onChange={handleChange('blocks')}
        placeholder="Block Kit JSON (if using blocks format)"
        sx={{ mb: 2 }}
      />

      <Handle type="source" position="bottom" isConnectable={isConnectable} />
    </Box>
  );
};

export default SlackNode;
